...
def verify_backend_token(token: str) -> dict:
    ...
    payload = jwt.decode(token, settings.BACKEND_JWT_SECRET, algorithms=["HS256"])
...
```env
BACKEND_JWT_SECRET="<same value as JWT_SECRET in Node.js backend>"
```
**File:** `F:\MasarX_A\src/helpers/config.py` — Add `BACKEND_JWT_SECRET: Optional[str] = None`
